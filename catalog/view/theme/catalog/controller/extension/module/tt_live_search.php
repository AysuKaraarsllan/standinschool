...
class ControllerExtensionModuleTTLiveSearch extends Controller {
	public function index() {
		...
		$order         = 'ASC';
		$page          = 1;
		$limit         = $this->config->get('module_tt_live_search_limit');
		$search_result = 0;
		$error         = false;
		...
		if(!$error){
			if (isset($this->request->get['filter_name'])) {
				...
				$image_width        = $this->config->get('module_tt_live_search_image_width') ? $this->config->get('module_tt_live_search_image_width') : 0;
				$image_height       = $this->config->get('module_tt_live_search_image_height') ? $this->config->get('module_tt_live_search_image_height') : 0;
				$title_length       = $this->config->get('module_tt_live_search_title_length');
				$description_length = $this->config->get('module_tt_live_search_description_length');
