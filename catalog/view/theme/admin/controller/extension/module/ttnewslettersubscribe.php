...
class ControllerExtensionModuleTTNewslettersubscribe extends Controller {
	...
	public function index() {   
		...
		$this->load->language('extension/module/ttnewslettersubscribe');

		$this->document->setTitle($this->language->get('page_title'));
		
		$this->load->model('setting/setting');
		$this->load->model('setting/module');
		$this->load->model('templatetrip/ttnewslettersubscribe');
		
		$this->model_templatetrip_ttnewslettersubscribe->check_db();
		$this->document->addScript('view/javascript/newsletter/jquery-ui.js');
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
	
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('ttnewslettersubscribe', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);			
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$user_total = $this->model_templatetrip_ttnewslettersubscribe->getTotalUsers();
		...
		$results = $this->model_templatetrip_ttnewslettersubscribe->getList($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('extension/module/ttnewslettersubscribe/unsubscribe', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . $result['id'] . $url, true)
			);
		...
		$pagination->url = $this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('extension/module/ttnewslettersubscribe', $data));
	...
	public function unsubscribe() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $user_id) {
				$this->model_templatetrip_ttnewslettersubscribe->delete($user_id);                
	  		}
			...
			$this->redirect($this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'] . $url, true));
			
		} else if( isset($this->request->get['user_id']) ) {
			
			$data = $this->request->get['user_id'];
			
			$this->model_templatetrip_ttnewslettersubscribe->delete($data);
			...
			$this->response->redirect($this->url->link('extension/module/ttnewslettersubscribe', 'user_token=' . $this->session->data['user_token'] . $url, true));
	...
	public function send() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if (!$this->user->hasPermission('modify', 'extension/module/ttnewslettersubscribe')) {
			...
			if (!$json) {
				...
				switch ($this->request->post['to']) {
					...
					case 'sendall':
						...
						$email_total1 = $this->model_templatetrip_ttnewslettersubscribe->getTotalUsers();
							
						$results1 = $this->model_templatetrip_ttnewslettersubscribe->getList($customer_data);
					...
					case 'lbnewsletter':
						...
						$email_total = $this->model_templatetrip_ttnewslettersubscribe->getTotalUsers();
							
						$results = $this->model_templatetrip_ttnewslettersubscribe->getList($customer_data);
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('extension/module/ttnewslettersubscribe/send', 'user_token=' . $this->session->data['user_token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
